...
12. For MCP definition changes, verify downstream state with `opencode mcp list` and `claude mcp list` (or `/mcp` in Claude), then report auth status and any required user action.
